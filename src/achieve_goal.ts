 ...
 */
export class GetUpdateActionVisitor extends DefaultResourceVisitor<
  (r: Resource) => Promise<Response>
> {
 ...
 */
export class GetUpdateTargetVisitor extends DefaultResourceVisitor<
  Resource | undefined
> {
  constructor() {
    super((r) => r);
...
export class CompareAndUpdateAction {
  ...
  public async execute(current: Resource, goal: Resource): Promise<Boolean> {
    if (
      JSON.stringify(current.getLocation()) !==
      JSON.stringify(goal.getLocation())
    ) {
...
export async function achieveGoal(
  current: Resource[],
  goal: Resource[],
  compareAndUpdateAction: CompareAndUpdateAction,
): Promise<number> {
  if (current.length != goal.length) {
    throw new Error(
      `Mismatched lengths for current and goal (${current.length}, ${goal.length})`,
    );
  }
