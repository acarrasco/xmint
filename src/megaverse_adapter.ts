...
export class MegaverseAdapter {
  ...
  public async getMapResources(): Promise<Resource[]> {
    ...
    return this.transformMatrix(
      mapResponse.map.content,
      this.mapCellToResource,
    );
  }
   ...
   */
  protected transformMatrix<G, T>(
    matrix: G[][],
    transform: (i: number, j: number, g: G) => T,
  ): T[] {
    const result: T[] = [];
  ...
  protected mapCellToResource(
    ...
    switch (mapCell?.type) {
      ...
      case 0:
        return new Polyanet(row, column);
      case 1:
        return new Soloon(row, column, mapCell.color);
      case 2:
        return new Cometh(row, column, mapCell.direction);
    }
  }
}
